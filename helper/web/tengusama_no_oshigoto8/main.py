    ...
    for file in PATH_CURRENT.glob("*.htm"):
        ...
        for table in tables:
            ...
            for row in rows:
                ...
                if len(cols) < 7 or "サークル名" in cols[0].get_text() or not cols[0].get_text(strip=True):
                    print(len(cols), cols)
                    continue
